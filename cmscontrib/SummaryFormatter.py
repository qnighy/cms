...
{% for user in users %}
    ...
    {% for task in user["tasks"] %}
        ...
        {% for testcase in task["testcases"] %}
            ...
            \\makebox[14mm]{ {{ format_size(testcase["execution_memory"]).replace(" ","") }} }
            {% end %}
        {% end %}
        \\end{description}
            ...
            for user in session.query(User)\
                ...
                for task in session.query(Task)\
                    ...
                    if t_submission is None:
                        ...
                        parameters = json.loads(task.active_dataset.score_type_parameters)
                        ...
                        subtasks = json.loads(scoretype.compute_score(t_submission_r)[1])
                ...
                user_data["max_score"] = round(max_score, contest.score_precision)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Outputs summary for each contestants.")
